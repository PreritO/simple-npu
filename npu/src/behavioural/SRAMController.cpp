...
void SRAMController::init() {
    init_SIM(); /* Calls the init of sub PE's and CE's */
}

std::string SRAMController::ExtractCorefromCluster(std::string input) {
...
void SRAMController::SRAMControllerThread(std::size_t thread_id) {
  while (1) {
    // 1. Read request
    ...
    if (auto mem_received = try_unbox_routing_packet<IPC_MEM>(received_tr)) {
      ...
      if (auto received_pd = try_unbox_routing_packet
        ...
        npu_error
        ("SRAMController "+setsourcetome_+" core section- cast failed");
      }
    ...
    //  to offchip memory (MCT)
      ...
      if (auto received_pd
        ...
        if (received_pd->command.find("FetchPKT") !=std::string::npos) {
          ...
          npu_error("SRAMController "+setsourcetome_+" ODE PD Invalid Command: "+received_pd->command)  // NOLINT(whitespace/line_length)
        }
...
void SRAMController::tlm_write_cumulative(tlm_data_type addr,
...
void SRAMController::tlm_read_cumulative(tlm_data_type addr,
 ...
 */
void
SRAMController::tlm_write(tlm_data_type addr, tlm_data_type datatowrite) {
...
SRAMController::tlm_data_type
SRAMController::tlm_read(tlm_data_type addr) {
