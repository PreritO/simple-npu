    ...
    class MemAwareExactMap : public bm::ExactLookupStructure {
        ...
        public:
            ...
            bool lookup(const bm::ByteContainer & key_data,
                ...
                //Commenting this out so it will compile, otherwise unchanged
                /*
                if (hashtrie) {
                ...
                */
               if (levelhash) {
                   Value v = levelhash->level_static_query
                        (byte_container_2_bitstring(key_data));
                    *handle = v.handle;
                    return v.match;
               } else {
            ...
            // Not touching this one, if used should seg fault to know that this is being used
            // don't think this is being referenced, leaving empty for now - PO
            ...
            // Currently dont think this is being used so commenting this out
            // but adding level_delete for completeness - PO
            void delete_entry(const bm::ExactMatchKey & key) override {
                ...
                */
               if (levelhash) {
                   levelhash->level_delete(byte_container_2_bitstring(key.data));
               } else {
                   cout << "Exact Level Hash could not delete - DS does not exist"
                        << __FILE__ << endl;
               }
            }
