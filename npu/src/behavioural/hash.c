...
uint64_t MurmurHash2 ( const void * key, uint64_t len, uint64_t seed ) {
...
uint64_t customHash(const void *data, uint64_t length, uint64_t seed)
{
/*
    ...
    */
    return MurmurHash2(data, length, seed);
}